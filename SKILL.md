...
displayName: 风鸟拓客线索挖掘
description: 通用 To B 拓客工具——按用户身份(卖什么)+目标客户类型(卖给谁)匹配行业经营范围词，用风鸟高级搜索 API 随机组合批量挖掘渠道商/集成商名单，自动补全电话邮箱。支持用户自主设定筛选条件 + AI 自适应丰富需求。免费、无需 API Key。
version: 1.0.0
license: MIT
---
...
通用 To B 拓客工具：告诉它「你是卖什么的」「卖给谁」，它自动匹配行业关键词、批量搜出目标渠道商/集成商名单，并补全电话邮箱。
